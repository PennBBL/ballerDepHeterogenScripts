...
####### Demographics ########
#############################
make_demographics_table(data_frame = subset_with_clusters_AG_matched, hydra_cluster = 3)
...
#Chi squared 
chisq_matched_sex <- chi_sq(data_frame = subset_with_clusters_AG_matched, "sex", 3)
chisq_matched_race <- chi_sq(data_frame = subset_with_clusters_AG_matched, "race_binarized", 3)

#get means/stds
total_people_Hydra_k3 <- total_people_per_cluster(data_frame = subset_with_clusters_AG_matched, hydra_cluster = 3)

num_men_all_clusters <- total_people_per_cluster_by_group(data_frame = subset_with_clusters_AG_matched, variable = "sex", hydra_cluster = 3, group_val = 1)
num_caucasian_all_clusters <- total_people_per_cluster_by_group(data_frame = subset_with_clusters_AG_matched, variable = "race_binarized", hydra_cluster = 3, group_val = 1)
...
network_cluster_stats_lm_agesq_AG_matched_by_cluster_1through10 <- lapply(clusters, function(cluster)
{
  ...
  {
    network <- as.name(network)
    lm(substitute(network ~ cluster + sex + ageSq + restRelMeanRMSMotion, list(network = network, cluster = cluster)), data = subset_with_clusters_AG_matched)
  })
...
{
  lm(substitute(i ~ Hydra_k3 + sex + ageSq + restRelMeanRMSMotion, list(i = as.name(network))), data = subset_with_clusters_AG_matched)
})
...
network_cluster_stats_anova_lm_agesq_AG_matched_by_cluster_1through10 <- lapply(clusters, function(cluster)
{
  ...
  {
     network <- as.name(network)
     anova(lm(substitute(network ~ cluster + sex + ageSq + restRelMeanRMSMotion, list(network = network, cluster = cluster)), data = subset_with_clusters_AG_matched))
  })
