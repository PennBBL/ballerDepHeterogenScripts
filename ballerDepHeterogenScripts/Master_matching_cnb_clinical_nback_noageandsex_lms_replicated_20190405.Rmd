---
title: "Master Replicated 0405019"
author: "Erica Baller"
date: "4/5/2019"
output:
...
7) Nback
-Using 21 functionally defined regions of interest from Satterthwaite et al, 2013, percent signal change between 2bk and 0bk was evaluated by cluster
- Results:
...
models_anova_lm_agesq <- lapply(cnb_score_cluster_stats_anova_matched, summary)

#Pull p-values
p_anova_lm_agesq <- sapply(cnb_score_cluster_stats_anova_matched, function(v) v$"Pr(>F)"[1]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
...
cnb_emmpairs_matched_FDR_corrected <- cnb_emmpairs_matched[c(cnb_fdr_anova_lm_agesq)]
...
{
#  lm(substitute(i ~ sex + age_in_years + ageSq + Hydra_k3, list(i = as.name(x))), data = subset_with_clusters_matched)
   lm(substitute(i ~ Hydra_k3, list(i = as.name(x))), data = subset_with_clusters_matched)
...
#model summary
models_anova_lm_agesq <- lapply(clinical_score_cluster_stats_anova_matched, summary)

#Pull p-values
p_anova_lm_agesq <- sapply(clinical_score_cluster_stats_anova_matched, function(v) v$"Pr(>F)"[1]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
...
#Only include stuff that was fdr corrected (i.e., only keep parts of the model (or only display) ones that are corrected)
clinical_emmpairs_matched_FDR_corrected <- clinical_emmpairs_matched[c(clinical_fdr_anova_lm_agesq)]
...
print(pairwise_table_with_fdr)
write.csv(pairwise_table_with_fdr, "/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/pairwise_table_BIFACTOR.csv")
...
print("Pairwise contrasts with FDR corrected values, Bifactor scores")
print(pairwise_table_with_fdr)
write.csv(pairwise_table_with_fdr, "/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/pairwise_table_ANXIETY.csv")
sapply(clinical_emmpairs_matched_FDR_corrected, function(x) {print(x)})
...
p_anova_task_active <- as.data.frame(p_anova[1:21,])
row.names(p_anova_task_active) <- row.names(p_anova)[1:21]
names(p_anova_task_active) <- c("p_anova_task_active")

#print BEFORE FDR correction 
print("LM N-back uncorrected")
print(p_anova)

print("LM anova scores task active areas, uncorrected")
print(p_anova_task_active)
...
pfdr_anova_task_active <- p.adjust(p_anova_task_active[,1],method="fdr")

#Convert to data frame
pfdr_anova <- as.data.frame(pfdr_anova)
pfdr_anova_task_active <- as.data.frame((pfdr_anova_task_active))

#keep Fs only for p's that were FDR corrected and less than 0.05 and zero out the rest

fs_fdr_corrected_with0s <- f_anova
fs_fdr_corrected_with0s[which(pfdr_anova >= 0.05),] <- 0 
...
#write csv for caret
#write.table(pfdr_anova, '/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/nback_fdr_corr_6rois_taskactive.csv', col.names = FALSE, quote=FALSE, sep=",")

#write.table(fs_fdr_corrected_with0s$f_anova,'/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/nback_f_fdr_corr0s_6rois_task_active.csv', col.names = FALSE, quote=FALSE, sep=",")

#write.table(fs_fdr_corrected, '/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/nback_f_fdr_corr_6rois_task_active.csv', col.names = FALSE, quote=FALSE, sep=",", row.names = FALSE)

#also include zeros for task_deactive areas
#write.table(c(fs_fdr_corrected_with0s$f_anova, rep(0,6)), '/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/nback_f_fdr_corr0s_6rois_fromFull27set.csv', col.names = FALSE, quote=FALSE, sep=",")

#change row names
row.names(pfdr_anova) <- parcellations
row.names(pfdr_anova_task_active) <- parcellations[1:21]

#To print fdr-corrected p-values to three decimal places
pfdr_round_anova <- round(pfdr_anova,3)
pfdr_round_anova_task_active <- round(pfdr_anova_task_active,3)

#List the components that survive FDR correction
parcellation_fdr_anova <- row.names(pfdr_anova)[pfdr_anova<0.05]
parcellation_fdr_anova_task_active <- row.names(pfdr_anova_task_active)[pfdr_anova_task_active<0.05]

#make a data frame with names and fdr values (rounded to 3 decimals)
parcellation_names_and_fdr_values_anova <- data.frame(cbind(parcellation_fdr_anova, round(pfdr_anova[pfdr_anova<0.05],4)))
parcellation_names_and_fdr_values_anova_task_active <- data.frame(cbind(parcellation_fdr_anova_task_active, round(pfdr_anova_task_active[pfdr_anova_task_active<0.05],4)))

#add titles to names_and_fdr tables
names(parcellation_names_and_fdr_values_anova) <- c("parcellation", "p_FDR_corr")
names(parcellation_names_and_fdr_values_anova_task_active) <- c("parcellation", "p_FDR_corr")

print("FDR corrected")
print(parcellation_names_and_fdr_values_anova)
print(parcellation_names_and_fdr_values_anova_task_active)
...
#contrast_table <- lapply(parcellation_emmpairs_AG_matched, function(x) {round(summary(x)$p.value,3)})
...
print ("LM pairwise contrasts and FDR corrrected values Jneurosci parcellations")
print(pairwise_table_with_fdr)

sapply(parcellation_emmpairs_AG_matched_FDR_corrected, function(x) {print(x)})
write.csv(pairwise_table_with_fdr, "/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/pairwise_table_NBACK.csv")



#### graph only the significant areas###
parcellation_task_active <- parcellation_names_and_fdr_values_anova_task_active$parcellation
