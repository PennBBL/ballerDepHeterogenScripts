---
title: "Nback Activation, matched age and sex, n368, Hydra K3, Schafer400 parcels"
author: "Erica Baller"
date: "8/10/2018"
output:
...
Part 1 : Read in csv.s
  -This script reads in demographics, Schaefer_scores, health and psych summaries, merges them, removes NAs, codes and separates by depression.
...
{
  parcellation = as.name(parcellation)
  gam(substitute(parcellation ~ Hydra_k3 + sex + s(age_in_years) + nbackRelMeanRMSMotion, list(parcellation = parcellation)), data = subset_with_clusters_AG_matched)
})
...
for(parcellation in parcellations){
  for(num in 1:total_num_groups) {
    clst <- groups[num,1]
    meas <- groups[num,2]
    mean_for_eval <- paste("mean(subset_with_clusters_AG_matched$", parcellation, "[which(subset_with_clusters_AG_matched$Hydra_k3 == ", groups[num,2], ")])", sep="")
    ...
    sd_for_eval <- paste("sd(subset_with_clusters_AG_matched$", parcellation, "[which(subset_with_clusters_AG_matched$Hydra_k3 == ", groups[num,2], ")])", sep="")
  ...
  #  all_mean_sd_sem$sem[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$parcellation == parcellation)] <- all_mean_sd_sem$sd[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$parcellation == parcellation)]/sqrt(nrow(subset_with_clusters_AG_matched))
    ...
    all_mean_sd_sem$sem[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$parcellation == parcellation)] <- all_mean_sd_sem$sd[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$parcellation == parcellation)]/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == meas)))
  }
}
...
ggplot(data = all_mean_sd_sem, aes(x = parcellation, y = mean, group = cl)) +  ylab("Community Connectivity") +
  geom_line(aes(color=cl)) +
  geom_point(aes(color=cl)) + 
  geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.1) +
  ggtitle("Hydra cluster by Schaefer Parcellation in Nback")
```
...
#print values
print("LM Agesq pairwise contrasts for FDR corrected values Cortical parcellation (MPRAGE JLF)")
print(pairwise_table)

print ("LM Agesq pairwise contrasts and FDR corr Cortical parcellation (MPRAGE JLF)")
...
#print values
print("LM Agesq pairwise contrasts UNCORRECTED values Cortical parcellation ")
print(pairwise_table)
