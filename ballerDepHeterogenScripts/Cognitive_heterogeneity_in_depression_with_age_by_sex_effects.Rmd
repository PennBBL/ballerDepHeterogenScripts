---
title: "Cognitive Heterogeneity in Depressed Youth"
author: "Erica Baller"
date: "5/14/2019"
output:
...
{
  lm(substitute(i ~ age_in_years*sex + Hydra_k3, list(i = as.name(x))), data = subset_with_clusters_matched)
})
...
#Pull p-values
p_anova <- sapply(cnb_score_cluster_stats_anova_matched, function(v) v$"Pr(>F)"[2]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
...
#Pull p-values
p_anova <- sapply(clinical_score_cluster_stats_anova_matched, function(v) v$"Pr(>F)"[2]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
