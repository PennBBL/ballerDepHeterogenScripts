---
title: "Cognitive Heterogeneity in Depressed Youth Age*group"
author: "Erica Baller"
date: "7/18/2019"
output:
...
3) HYDRA
- Final matched groups were output to csv and sent to HYDRA for subtyping (code protected)
...
- Results:
   ...
   - Subtype 3: Impulsive
- age * sex, significant only for lan_z and edi
...
- Results:
  ...
  - Subtype 1 had higher anxious-miser scores than Subtype 2 (P=0.030)
  - Subtype 2 had higher fear scores than Subtypes 1 and 3 (P(FDR)<0.0001)
- age * sex, no between subtype differences

6) Anxious-misery analysis
- Anxious misery factor scores were calculated 
- Subtypes were evaluated on state and trait factors to verify cognitive differences were not due to state difference
- Results:
...
- age * sex, no significant differences
...
{
  lm(substitute(i ~ age_in_years + Hydra_k3 + age_in_years:Hydra_k3, list(i = as.name(x))), data = subset_with_clusters_matched)
})
...
#Pull p-values
p_anova <- sapply(cnb_score_cluster_stats_anova_matched, function(v) v$"Pr(>F)"[3]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
...
#print values
print("LM pairwise contrasts for FDR corrected values, CNB scores")
...
#Pull p-values
p_anova <- sapply(clinical_score_cluster_stats_anova_matched, function(v) v$"Pr(>F)"[3]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
...
#Pull p-values
p_anova <- sapply(clinical_score_cluster_stats_anova_matched, function(v) v$"Pr(>F)"[2]) #get the p value for dep binarized
...
{
  lm(substitute(i ~ nbackRelMeanRMSMotion + age_in_years + Hydra_k3 + age_in_years:Hydra_k3, list(i = as.name(parcellation))), data = subset_with_clusters_matched)
})
