...
output_cnb <- lapply(cnb_measure_names, function(x) {
  lm(substitute(i ~ Hydra_k3, list(i=as.name(x))), data = cnb)
})
...
output_clinical <- lapply(clinical_measure_names, function(x) {
  lm(substitute(i ~ Hydra_k3, list(i=as.name(x))), data = clinical)
})
...
output_anxious_mis <- lapply(anxious_misery_names, function(x) {
  lm(substitute(i ~ Hydra_k3, list(i=as.name(x))), data = anxious_misery)
})
...
output_nback <- lapply(parcellations, function(x) {
  lm(substitute(i ~ nbackRelMeanRMSMotion + Hydra_k3, list(i=as.name(x))), data = nback)
})
...
output_network <- lm(network_strength~restRelMeanRMSMotion + nbackRelMeanRMSMotion + idemoRelMeanRMSMotion + Hydra_k3, data = networks)
