...
get_schaefer_netpath <- function(resolution, modality, bblid, scanid){
  ...
  else if (modality == 'nback') {
    netpath <- paste0(remote_wkdir, "processedData/nback/xcptaskregressed/", bblid, "/*", scanid, "/fcon/schaefer", resolution, "/*_network.txt")
  }
  netpath
}
...
get_net_from_sample <- function(sample,parcellation,resolution,modality) {
  ...
  for (i in 1:n_sample) {
    ...
    if (parcellation == 'power') {
      ...
      } else if (modality == 'fc' | modality == 'nback') {
        sample_net[[i]] <- grab_net_from_path(netpath) 
      }
    }
  }
...
#########################################
### Calculate mean per nback community###
########################################

#get_nback_mean_by_community <- function(sample, num_communities) {
  
#}
make_nback_community_matrix <- function(sample, resolution, num_communities){
  #convert sample to matrix
  #sample <- as.matrix(sample)
  #get the node affiliations and information
  num_subjs <- dim(sample)[1]
  nback_community_matrix_to_append <- matrix(NA,nrow = num_subjs, ncol = num_communities)
  colnames(nback_community_matrix_to_append) <- c(1:num_communities)
  schaefer_node_info <- get_schaefer_node_info(resolution = resolution, num_com = num_communities)
 
  for (subj in 1:num_subjs) {
    for (com in 1:num_communities) {
      sample_for_com <- sample[subj, which(schaefer_node_info$CommunityAffiliation == com)]
      nback_community_matrix_to_append[subj,com] <- as.numeric(rowMeans(sample_for_com[1,]))
    }
  }
  #sample_with_com_averages <- cbind(sample, nback_community_matrix_to_append)
  out <- list(mean = nback_community_matrix_to_append, data = sample)
  return(out)
...
#added 10/24, this allows me not to look at connectivity, but just activation
#get_community_net_nback <-function(netmat,com_name_vector,node_aff_vector){
 # community_net =  array(NA, dim=c(length(com_name_vector))
#  for (community_i in 1:length(com_name_vector)){
 #     nodes_in_community_i = which(node_aff_vector == community_i)
  #    community_net[community_i] = 
   #     mean(netmat[nodes_in_community_i],na.rm=TRUE)
      #community_pair_var = communnity_var[community_i,community_j]
      #community_net[community_i,community_j] = get_subj_mean_of_sqr(netmat,nodes_in_community_i,nodes_in_community_j) - community_pair_var
    #}
  #}
