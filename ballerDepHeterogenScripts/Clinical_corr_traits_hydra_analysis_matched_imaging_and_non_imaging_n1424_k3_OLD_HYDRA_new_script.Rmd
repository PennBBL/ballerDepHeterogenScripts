---
title: "Clinical_hydra_analysis_matched_imaging_and_non_imaging_n1424_k3, regressed_on_age_and_sex,CORR TRAITS, OLD Hydra, new script"
author: "Erica Baller"
date: "11/21/2018"
output:
...
This script goes through demographics, clinical scores (corr traits), health, and psych summaries, adds clustering information, runs statistics and makes graphs from results.
...
Part 6 : Anovas
  -Anovas were also run on the results of the LM of each clinical corr trait value by cluster.


Part 7 : FDR Correction
  -FDR correction was calculated for each clinical corr trait measure ANOVA output
...
#clinical_scores <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/data/n9498_goassess_itemwise_corrtraits_scores_age_regressed_20161219.csv", header = TRUE, sep = ",") 
clinical_scores <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/data/n9498_goassess_itemwise_corrtraits_scores_20161219.csv", header = TRUE, sep = ",") 
...
clinical_measure_names <- names(subset_dep_or_no_psych_and_no_medicalratingExclude_DEPBINARIZED)[grep("corrtraits", names(subset_dep_or_no_psych_and_no_medicalratingExclude_DEPBINARIZED))] #get the names of all the columns with corrtraits in the name
cluster_names <- colnames(hydra_AG_matched_clusters[,2:11])

clinical_measure_names_list <- names(subset_dep_or_no_psych_and_no_medicalratingExclude_DEPBINARIZED)[grep("corrtraits", names(subset_dep_or_no_psych_and_no_medicalratingExclude_DEPBINARIZED))] #get the names of all the columns with _z in the name
...
############################################
groups_list <- data.frame(cbind(c("TD", "Cluster 1", "Cluster 2", "Cluster 3"), c("-1", "1", "2", "3")))
names(groups_list) <- c("cl", "numeric")
all_mean_sd_sem <- data.frame(rep(groups_list$cl, 4), rep(clinical_measure_names, each = 4), rep(0, 1), rep(0, 1), rep(0,1))
...
for(clinical in clinical_measure_names_list){
  for(num in 1:4) {
    ...
    mean_for_eval <- paste("mean(subset_with_clusters_AG_matched$", clinical, "[which(subset_with_clusters_AG_matched$Hydra_k3 == ", groups_list[num,2], ")])", sep="")
...
#remove _4corrtraits from names
...
#  ggtitle("Hydra_k3 Clinical Measures: Correlated Traits") 

names(all_mean_sd_sem) <- c("cluster", "clinical", "mean", "sd", "sem")
ggplot(data = all_mean_sd_sem, aes(x = clinical, y = mean, group = cluster)) + ylab("Mean Z-score")  +
  ggtitle(paste0("Clinical Measures: Correlated Traits for Hydra-", num_clusters, " cluster solution")) + 
 ...
 # scale_x_discrete(limits=all_mean_sd_sem$clinical, labels=all_mean_sd_sem$new_names, name = " ") +
  scale_x_discrete(limits=all_mean_sd_sem$clinical, name = "") +
...
#get the corrtraits out of the name
fdr_corrected_clinical_gsub <- gsub("_corrtraitsv2", "", fdr_corrected_clinical)
...
print("LM Agesq pairwise contrasts for FDR corrected values, corr traits")
print(pairwise_table)

print("LM Agesq pairwise contrasts with FDR corrected values, corr traits")
