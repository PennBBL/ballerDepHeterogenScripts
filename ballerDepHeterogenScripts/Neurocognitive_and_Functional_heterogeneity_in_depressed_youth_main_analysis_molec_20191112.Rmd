...
#start with number of subjs per group
num_per_cluster <- total_people_per_cluster(subset_with_clusters_matched, 3)

#get cohen's effect sizes per parcellation

parcellation_eff_size <- lapply(parcellations, function(parcellation) {
mean_sd_sem <- data_frame_mean_sd_sem(data_frame = subset_with_clusters_matched, variable = parcellation, hydra_cluster = num_clusters)

#send mean_sd_sem matrix and number of people per cluster
cohen_d_Hydra_k3(mean_sd_sem, num_per_cluster)
})
names(parcellation_eff_size) <- parcellations

#build table that will hold the name of the brain region and the p values
eff_size_table <- data.frame(matrix(nrow = 6, ncol = 6))
rownames(eff_size_table) <- parcellations
colnames(eff_size_table) <- contrast_names

#loop through each brain region, and manually assign the columns to be the p values
for (region in parcellations)
{
  pair_eff <- parcellation_eff_size[[region]]
  eff_size_table[region,] <- round(pair_eff,3)
}
eff_size_table
