...
set currdir = `pwd`
set wkdir = "/data/jux/BBL/projects/ballerDepHeterogen/data/neuroimaging/processed_data/concat_restbold_nback_idemo/rest_nback_idemo_8FunctionalROIs_2mm/"
...
set newts = $wkdir/ts_files_from_3dROIstats_8rois_2mm
...
foreach ts (`ls $oldts/*.txt`)
	...
	cp $ts ${newts}/${ts_root}_8roi_2mm.txt
	#change format of new file so it is single space delimited
	more ${newts}/${ts_root}_8roi_2mm.txt | sed -e "s/\s\{1,\}/ /g" >! temp
	mv temp ${newts}/${ts_root}_8roi_2mm.txt	
	Rscript call_27roi_to_8_roi_r_script_2mm.R ${newts}/${ts_root}_8roi_2mm.txt
end
