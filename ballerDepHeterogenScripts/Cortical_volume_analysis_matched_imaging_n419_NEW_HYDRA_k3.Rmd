---
title: "Cortical_volume_analysis_matched_age_and_sex_imaging_k3, NEW HYDRA, n 419"
author: "Erica Baller"
date: "10/23/2018"
output:
...
volume_score_cluster_stats_lm_AG_matched_by_cluster_1through10 <- lapply(cluster_names_list, function(cluster_name)
{
  volume_score_cluster_stats_lm_AG_matched_withincluster<- lapply(volume_measure_names_list, cluster=as.name(cluster_name), function(volume_measure_name, cluster) 
  {
    ...
    lm(substitute(volume_measure ~ cluster + sex + age_in_years + averageManualRating, list(volume_measure = volume_measure, cluster = cluster)), data = subset_with_clusters_AG_matched)
  })
...
volume_score_cluster_stats_gam_AG_matched_by_cluster_1through10 <- lapply(cluster_names_list, function(cluster_name)
{
 ...
 {
    volume_measure <- as.name(volume_measure_name)
    gam(substitute(volume_measure ~ cluster + sex + s(age_in_years) + averageManualRating, list(volume_measure = volume_measure, cluster = cluster)), data = subset_with_clusters_AG_matched, method="REML")
  })
...
#lm
volume_score_cluster_stats_lm_AG_matched <- lapply(volume_measure_names_list, function(x) 
{
  lm(substitute(i ~ Hydra_k3 + sex+age_in_years+ averageManualRating, list(i = as.name(x))), data = subset_with_clusters_AG_matched)
})
names(volume_score_cluster_stats_lm_AG_matched) <- volume_measure_names

#gam
volume_score_cluster_stats_gam_AG_matched <- lapply(volume_measure_names_list, function(volume_measure) 
{
  volume_measure = as.name(volume_measure)
  gam(substitute(volume_measure ~ Hydra_k3 + sex + s(age_in_years) + averageManualRating, list(volume_measure = volume_measure)), data = subset_with_clusters_AG_matched, method="REML")
})
names(volume_score_cluster_stats_gam_AG_matched) <- volume_measure_names

#lm ageSq and mean centered
volume_score_cluster_stats_lm_agesq_AG_matched <- lapply(volume_measure_names_list, function(x) 
{
...
volume_score_cluster_stats_anova_AG_matched_by_cluster_1through10 <- lapply(cluster_names_list, function(cluster_name)
{
  ...
  {
     volume_measure <- as.name(volume_measure_name)
     anova(lm(substitute(volume_measure ~ cluster + sex + age_in_years + averageManualRating, list(volume_measure = volume_measure, cluster = cluster)), data = subset_with_clusters_AG_matched))
  })
...
volume_score_cluster_stats_anova_gam_AG_matched_by_cluster_1through10 <- lapply(cluster_names_list, function(cluster_name)
{
  ...
  {
     volume_measure <- as.name(volume_measure_name)
     anova(gam(substitute(volume_measure ~ cluster + sex + s(age_in_years) + averageManualRating, list(volume_measure = volume_measure, cluster = cluster)), data = subset_with_clusters_AG_matched, method="REML"))
  })
