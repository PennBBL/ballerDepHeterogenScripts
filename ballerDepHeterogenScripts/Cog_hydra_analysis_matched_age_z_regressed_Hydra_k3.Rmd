...
ggplot(dat_sex_race_no_significance, aes(x = cl, y = percent_males, fill=cl)) + geom_col() +
  ...
  ggtitle("Hydra_k3 % Male, p = 0.1.92x10-15") + scale_fill_discrete(breaks=c("TD", "Cluster1", "Cluster2", "Cluster3")) +
...
#all_mean_sd_sem <- data.frame(rep(groups_list$cl, 26), rep(cnb_measure_names, 4), rep(0, 1), rep(0, 1), rep(0,1))
 ...
 # for(num in 1:4) {
  #  clst <- groups_list[num,1]
   # meas <- groups_list[num,2]
    #print(clst)
    #print(meas)
    #txt <- paste("mean(subset_with_clusters_AG_matched$", cnb, "[which(subset_with_clusters_AG_matched$Hydra_k3 == ", groups_list[num,2], ")])", sep="")
    #mean_grp <- eval(parse(text=as.name(txt)))
    #print(mean_grp)  
    #print(txt)
    #df_mean <- which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$cnb == meas)
   # print(df_mean)
    #df$aged <= df$laclen, 
    #row_num <- subset(x = all_mean_sd_sem, subset = (all_mean_sd_sem[which(all_mean_sd_sem$cl == groups_list[num,1])] & all_mean_sd_sem[which(all_mean_sd_sem$cnb == groups_list[num,2])]))
    #print(row_num)
  #  all_mean_sd_sem$mean[]
    #ct <-groups_list[num,2]
    #if (ct == "-1"){
     #print("YES")
  #  }
      
    #print(ct)
   #print(groups_list$numeric[count])
 # }
#}
    #df_mean <- paste("df_mean_", cnb, "<- c(mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == -1)]),",
     #              " mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 1)]),",
      #             " mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 2)]),",
       #            " mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 3)]))", sep="")
    #eval(parse(text=as.name(df_mean)))
#  }
#}
    
  #cnb_short <- gsub("_4factorv2", "", x = cnb)
  #make columns for means of each cnb group
  #df_text <- paste("df_mean_", cnb, "<- c(mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == -1)]),",
   #                " mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 1)]),",
    #               " mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 2)]),",
     #              " mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 3)]))", sep="")
  ...
  #df_sd_sem <- paste("cnb_data_", cnb, "_sd_sem <- data.frame(cl = c(\"TD\", \"Cluster1\", \"Cluster2\", \"Cluster3\"), mean_", cnb,
   #                  "= c(mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == -1)]),",
    #                 "mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 1)]),", 
     #                "mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 2)]),", 
      #               "mean(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 3)])),",
       #              "sd_", cnb, "= c(sd(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == -1)]),",
        #             "sd(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 1)]),",
         #            "sd(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 2)]),",
          #           "sd(subset_with_clusters_AG_matched$", cnb,"[which(subset_with_clusters_AG_matched$Hydra_k3 == 3)])))", sep = "")
  #eval(parse(text=as.name(df_sd_sem)))
  
  #add sem to each
  #sem <- paste("cnb_data_", cnb, "_sd_sem$sem <- cnb_data_", cnb, "_sd_sem$sd_", cnb, "/sqrt(nrow(subset_with_clusters_AG_matched))", sep = "")
...
#make a data frame with all standard errors
#all_sem
#all_sem <- data.frame(cnb_data_mood_sd_sem$sem, cnb_data_psychosis_sd_sem$sem, cnb_data_phobias_sd_sem$sem, cnb_data_externalizing_sd_sem$sem, cnb_data_overall_psychopathology_sd_sem$sem)
#make a single column of sem in the data frame mood (TD, Cluster1, Cluster 2, Cluster3), psychosis(TD, cluster 1, cluster 2), etc
#all_sem_one_col <- data.frame(sem=unlist(all_sem, use.names = FALSE))

#cnb_all_measures <- data.frame(cl=c("TD", "Cluster1", "Cluster2", "Cluster3"), mood =c(df_mean_mood), psychosis = c(df_mean_psychosis), phobias = c(df_mean_phobias), extern = c(df_mean_externalizing), overall = c(df_mean_overall_psychopathology))
