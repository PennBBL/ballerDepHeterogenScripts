#Hydra_k3 last in model (all covariates that come before in the model are accounted for in the estimates of Hydra_k3)
...
output_cnb <- lapply(cnb_measure_names, function(x) {
  lm(substitute(i ~ sex + age_in_years + ageSq + Hydra_k3, list(i=as.name(x))), data = cnb)
})
names(output_cnb) <- cnb_measure_names
cnb_summary <- lapply(output_cnb, summary)
output_cnb_anova <- lapply(output_cnb, anova)
output_cnb_pval <- as.data.frame(sapply(output_cnb_anova, function(x) x$"Pr(>F)"[4]))
...
output_clinical <- lapply(clinical_measure_names, function(x) {
  lm(substitute(i ~ sex + age_in_years + ageSq + Hydra_k3, list(i=as.name(x))), data = clinical)
})
...
output_clinical_pval <- as.data.frame(sapply(output_clinical_anova, function(x) x$"Pr(>F)"[4]))
...
output_anxious_mis <- lapply(anxious_misery_names, function(x) {
  lm(substitute(i ~ sex + age_in_years + ageSq + Hydra_k3, list(i=as.name(x))), data = anxious_misery)
})
...
output_am_pval <- as.data.frame(sapply(output_am_anova, function(x) x$"Pr(>F)"[4]))
...
output_nback <- lapply(parcellations, function(x) {
  lm(substitute(i ~ sex + age_in_years + ageSq + nbackRelMeanRMSMotion + Hydra_k3, list(i=as.name(x))), data = nback)
})
names(output_nback) <- parcellations
nback_summary <- lapply(output_nback, summary)
output_nback_anova <- lapply(output_nback, anova)
output_nback_pval <- as.data.frame(sapply(output_nback_anova, function(x) x$"Pr(>F)"[5]))
...
output_network <- lm(network_strength ~ sex + age_in_years + ageSq + restRelMeanRMSMotion + nbackRelMeanRMSMotion + idemoRelMeanRMSMotion + Hydra_k3, data = networks)
network_summary <- summary(output_network)
output_network_anova <- anova(output_network)
output_network_pval <- output_network_anova$"Pr(>F)"[7]
