---
title: "Functional Connectivity, weighted removal, concatenated(rest+nback+idemo), r400, com17 communities, n 312, Hydra_k3"
author: "Erica Baller"
date: "7/31/18"
output:
...
filename <- Sys.glob(paste0("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/output_of_fc/concatenate_rest_nback_idemo/fc_concat_", exclusion, "_r", res, "_com", num_communities, "/fc_concat_", exclusion, "_r", res, "_com", num_communities, ".csv"))
...
new_hydra_clusters <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/hydra_output_from_cbica/hydra_matched/20181016_New_Hydra/CogData_matched_age_and_sex_n1424_matched_imaging_plus_non_imaging_replication_20181016_HydraSubtypes.csv", header = TRUE, sep = ",")
...
##### End of new material from 10/21########


#set up within/between networks
networks_within <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/community_names.csv")
within <- networks_within[2]
...
network_cluster_stats_lm_AG_matched_by_cluster_1through10 <- lapply(clusters, function(cluster)
{
  network_cluster_stats_lm_AG_matched_withincluster<- lapply(networks, cluster=as.name(cluster), function(network, cluster) 
  {
    ...
    lm(substitute(network ~ cluster + sex + age_in_years + restRelMeanRMSMotion, list(network = network, cluster = cluster)), data = subset_with_clusters_AG_matched)
  })
...
network_cluster_stats_gam_AG_matched_by_cluster_1through10 <- lapply(clusters, function(cluster)
{
 ...
 {
    network <- as.name(network)
    gam(substitute(network ~ cluster + sex + s(age_in_years) + restRelMeanRMSMotion, list(network = network, cluster = cluster)), data = subset_with_clusters_AG_matched, method = "REML")
  })
...
{
  lm(substitute(i ~ Hydra_k3 + sex + age_in_years + restRelMeanRMSMotion, list(i = as.name(network))), data = subset_with_clusters_AG_matched)
})
...
{
  network = as.name(network)
  gam(substitute(network ~ Hydra_k3 + sex + s(age_in_years) + restRelMeanRMSMotion, list(network = network)), data = subset_with_clusters_AG_matched, method = "REML")
})
...
{
  lm(substitute(i ~ Hydra_k3 + sex + age_in_years + restRelMeanRMSMotion, list(i = as.name(network))), data = subset_with_clusters_AG_matched_noTD)
})
...
{
  network = as.name(network)
  gam(substitute(network ~ Hydra_k3 + sex + s(age_in_years) + restRelMeanRMSMotion, list(network = network)), data = subset_with_clusters_AG_matched_noTD, method = "REML")
})
...
{
  lm(substitute(i ~ Hydra_k3 + sex + age_in_years + ageSq + restRelMeanRMSMotion, list(i = as.name(network))), data = subset_with_clusters_AG_matched_noTD)
})
...
network_cluster_stats_anova_AG_matched_by_cluster_1through10 <- lapply(clusters, function(cluster)
{
  ...
  {
     network <- as.name(network)
     anova(lm(substitute(network ~ cluster + sex + age_in_years + restRelMeanRMSMotion, list(network = network, cluster = cluster)), data = subset_with_clusters_AG_matched))
  })
...
network_cluster_stats_anova_gam_AG_matched_by_cluster_1through10 <- lapply(clusters, function(cluster)
{
  ...
  {
     network <- as.name(network)
     anova(gam(substitute(network ~ cluster + sex + s(age_in_years) + restRelMeanRMSMotion, list(network = network, cluster = cluster)), data = subset_with_clusters_AG_matched, method = "REML"))
  })
...
cluster_titles<- get_cluster_titles_no_TD(hydra_cluster = num_clusters)
cluster_vector <- get_cluster_numerical_vector_no_TD(hydra_cluster = num_clusters)
groups <- data.frame(cbind(cluster_titles, cluster_vector))
names(groups) <- c("cl", "numeric")
total_num_groups <- num_clusters
...
df_mean_sd_sem <- NULL
for(network in networks) {
  mean_sd_sem <- data_frame_mean_sd_sem_no_TD(data_frame = subset_with_clusters_AG_matched, variable = network, hydra_cluster = num_clusters)
  df_mean_sd_sem <- rbind(df_mean_sd_sem, mean_sd_sem)
}
...
for(network in networks){
  ...
  #  all_mean_sd_sem$sem[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)] <- all_mean_sd_sem$sd[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)]/sqrt(nrow(subset_with_clusters_AG_matched))
    ...
    #THIS WAS CHANGED 8/8, sem should be based on the number of subjects in each group!
    all_mean_sd_sem$sem[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)] <- all_mean_sd_sem$sd[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)]/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == as.numeric(meas))))
  }
}
...
ggplot(data = all_mean_sd_sem, aes(x = network, y = mean, group = cl)) +  ylab("Community Connectivity") +
  ...
  ggtitle("Hydra cluster by Network Connectivity No TD")
...
for(network in within){
  ...
  #  all_mean_sd_sem$sem[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)] <- all_mean_sd_sem$sd[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)]/sqrt(nrow(subset_with_clusters_AG_matched))
    ...
    #THIS WAS CHANGED 8/8, sem should be based on the number of subjects in each group!
    all_mean_sd_sem_within$sem[which(all_mean_sd_sem_within$cl == clst & all_mean_sd_sem_within$network == network)] <- all_mean_sd_sem_within$sd[which(all_mean_sd_sem_within$cl == clst & all_mean_sd_sem_within$network == network)]/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == as.numeric(meas))))
  }
}
...
ggplot(data = all_mean_sd_sem_within, aes(x = network, y = mean, group = cl)) +  ylab("Community Connectivity") +
  ...
  geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.1) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Hydra cluster by WITHIN Network Connectivity No TD")
...
for(network in between){
  ...
  #  all_mean_sd_sem$sem[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)] <- all_mean_sd_sem$sd[which(all_mean_sd_sem$cl == clst & all_mean_sd_sem$network == network)]/sqrt(nrow(subset_with_clusters_AG_matched))
    ...
    #THIS WAS CHANGED 8/8, sem should be based on the number of subjects in each group!
    all_mean_sd_sem_between$sem[which(all_mean_sd_sem_between$cl == clst & all_mean_sd_sem_between$network == network)] <- all_mean_sd_sem_between$sd[which(all_mean_sd_sem_between$cl == clst & all_mean_sd_sem_between$network == network)]/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == as.numeric(meas))))
  }
}
...
ggplot(data = all_mean_sd_sem_between, aes(x = network, y = mean, group = cl)) +  ylab("Community Connectivity") +
  ...
  geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.1) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Hydra cluster by BETWEEN Network Connectivity no TD")
...
```{r FDR Correction, no TD, echo = FALSE, fig.width=10}
...
#Looking at Hydra_k3, but can redo with nested list
#Look at model summaries
models_anova <- lapply(network_cluster_stats_anova_lm_AG_matched_noTD, summary)

#Pull p-values
p_anova <- sapply(network_cluster_stats_anova_lm_AG_matched_noTD, function(v) v$"Pr(>F)"[1]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized

#Convert to data frame
p_anova <- as.data.frame(p_anova)

#print BEFORE FDR correction 
print("LM anova scores, BEFORE FDR correction, i.e. uncorrected, NO TD")
print(p_anova)
...
#Looking at Hydra_k3, but can redo with nested list
#Look at model summaries
#models_anova <- lapply(network_cluster_stats_anova_gam_AG_matched_noTD, summary)

#Pull p-values
p_anova <- sapply(network_cluster_stats_anova_gam_AG_matched_noTD, function(v) v$pTerms.table[1,3])
                    #"Pr(>F)"[1]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
...
#print BEFORE FDR correction 
print("GAM anova scores, BEFORE FDR correction, i.e.- uncorrected, no TD")
print(p_anova)
...
#Looking at Hydra_k3
#Look at model summaries
models_anova_lm_agesq <- lapply(network_cluster_stats_anova_lm_agesq_AG_matched_noTD, summary)

#Pull p-values
p_anova_lm_agesq <- sapply(network_cluster_stats_anova_lm_agesq_AG_matched_noTD, function(v) v$"Pr(>F)"[1]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized

#Convert to data frame
p_anova_lm_agesq <- as.data.frame(p_anova_lm_agesq)

#print BEFORE FDR correction 
print("LM Agesq anova scores, BEFORE FDR correction: i.e., uncorrected, no TD")
print(p_anova_lm_agesq)
...
print("LM Agesq- Mean centered age that was then squared, FDR corrected, no TD")
print(network_names_and_fdr_values_anova_lm_agesq)

#####separating within and between connectivity

within_lm <- network_cluster_stats_lm_agesq_AG_matched_noTD[within]
between_lm <- network_cluster_stats_lm_agesq_AG_matched_noTD[!is.element(network_cluster_stats_anova_lm_agesq_AG_matched_noTD, within_lm)]

within_anova <- network_cluster_stats_anova_lm_agesq_AG_matched_noTD[within]
between_anova<- network_cluster_stats_anova_lm_agesq_AG_matched_noTD[!is.element(network_cluster_stats_anova_lm_agesq_AG_matched_noTD, within_anova)]

within_fdr <- fdr_anova(data_frame = within_anova)
between_fdr <- fdr_anova(data_frame = between_anova)

print("FDR corrected- within communities, no TD")
print(within_fdr)

print("FDR corrected - between communities, no TD")
print(between_fdr)

print("Pairwise contrasts, within communities, fdr_corrected, no TD")
pair_output_within <- pairwise_contrasts_3clusters(within_lm, within_fdr)
print(pair_output_within) #call 
#print(pairwise_contrasts_within_Hydra_k3)

print("Pairwise contrasts, between communities, fdr_corrected, no TD")
pair_output_between <- pairwise_contrasts_3clusters(between_lm, between_fdr)
print(pair_output_between) #call 
...
#put lm model into emmeans format
network_emmodel_lm_agesq_AG_matched_noTD <- lapply(network_cluster_stats_lm_agesq_AG_matched_noTD, function(x) {as.list(ref_grid(x))})
network_emmgrid_lm_agesq_AG_matched_noTD <- lapply(network_emmodel_lm_agesq_AG_matched_noTD, function(x) {as.emmGrid(x)})

#run emmeans
network_emmeans_lm_agesq_AG_matched_noTD <- lapply(network_emmgrid_lm_agesq_AG_matched_noTD, function(x) {emmeans(x, "Hydra_k3")})

#run pairwise contrasts
network_emmpairs_lm_agesq_AG_matched_noTD <- lapply(network_emmeans_lm_agesq_AG_matched_noTD, function(x) {pairs(x)})

#Only include stuff that was fdr corrected (i.e., only keep parts of the model (or only display) ones that are corrected),this will be null if nothing was corrected
network_emmpairs_lm_agesq_AG_matched_noTD_FDR_corrected <- network_emmpairs_lm_agesq_AG_matched_noTD[c(network_fdr_anova_lm_agesq)]
...
contrast_names <- c("-1 - 1", "-1 - 2", "-1 - 3", "1 - 2", "1 - 3", "2 - 3")
#go through each fdr corrected brain region, and extract p values
contrast_table <- lapply(network_emmpairs_lm_agesq_AG_matched_noTD_FDR_corrected, function(x) {round(summary(x)$p.value,3)})
#get the names of the brain regions that were fdr corrected
fdr_corrected_brain_regions <- names(contrast_table)
#build table that will hold the name of the brain region and the p values
pairwise_table <- data.frame(matrix(nrow = length(fdr_corrected_brain_regions), ncol = 6))
...
#print values
print("LM Agesq pairwise contrasts for FDR corrected values Cortical network, no TD")
print(pairwise_table)

print ("LM Agesq pairwise contrasts and FDR corr Cortical network , no TD")
print(pairwise_table_with_fdr)

sapply(network_emmpairs_lm_agesq_AG_matched_noTD_FDR_corrected, function(x) {print(x)})
...
contrast_names <- c("-1 - 1", "-1 - 2", "-1 - 3", "1 - 2", "1 - 3", "2 - 3")
#go through each fdr corrected brain region, and extract p values
contrast_table <- lapply(network_emmpairs_lm_agesq_AG_matched_noTD, function(x) {round(summary(x)$p.value,3)})
#get the names of the brain regions that were fdr corrected
brain_regions <- names(contrast_table)
#build table that will hold the name of the brain region and the p values
pairwise_table <- data.frame(matrix(nrow = length(brain_regions), ncol = 6))
...
sapply(network_emmpairs_lm_agesq_AG_matched_noTD, function(x) {print(x)})
...
#checkmodel with visreg, uncomment when want to check####
#########################################################
#invisible(lapply(network_cluster_stats_lm_AG_matched_noTD, function(x) {visreg(x)}))
