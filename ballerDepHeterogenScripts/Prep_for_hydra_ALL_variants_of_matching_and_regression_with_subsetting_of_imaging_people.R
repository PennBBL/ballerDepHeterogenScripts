...
#only include good imaging scans
...
############################
for(vers in matched_versions) {
  merge_string_to_eval <- paste("merge(", vers, ".matched, imaging_include, by = \"bblid\")", sep = "")
  merged_data <- eval(parse(text=as.name(merge_string_to_eval)))
  #demo_string_to_eval <- paste("data.frame(", vers , ".matched$race_binarized, ", vers, ".matched$sex, ", vers, ".matched$medu1, ", vers, ".matched$age_in_years, ", vers, ".matched$dep_binarized)", sep ="")
  #demo_data.matched <- eval(parse(text=as.name(demo_string_to_eval)))
  demo_data.matched <- data.frame(merged_data$race_binarized, merged_data$sex, merged_data$medu1, merged_data$age_in_years, merged_data$dep_binarized)
  names(demo_data.matched) <- c(listVars)
