---
title: "Master file with RDS, and no age and sex in lms "
author: "Erica Baller"
date: "3/19/2019"
output:
...
subset_with_clusters_AG_matched <- readRDS("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/rds/cnb_results.rds")
listVars <- c("Race", "Sex", "Maternal Ed", "Age", "Depression") #Race 1 = caucasian, Maternal Ed = years, age = years, dep 1 = dep, 0 = non_dep

demo <- data.frame(subset_with_clusters_AG_matched$race_binarized, subset_with_clusters_AG_matched$sex, subset_with_clusters_AG_matched$medu1, subset_with_clusters_AG_matched$age_in_years, subset_with_clusters_AG_matched$dep_binarized)
...
cnb_measure_names <- names(subset_with_clusters_AG_matched)[grep("_z", names(subset_with_clusters_AG_matched))] #get the names of all the columns with _z in the name
...
cnb_measure_names_list <- names(subset_with_clusters_AG_matched)[grep("_z", names(subset_with_clusters_AG_matched))] #get the names of all the columns with _z in the name


#lm ageSq and mean centered
cnb_score_cluster_stats_lm_agesq_AG_matched <- lapply(cnb_measure_names, function(x) 
{
#  lm(substitute(i ~   Hydra_k3 + age_in_years + ageSq + sex, list(i = as.name(x))), data = subset_with_clusters_AG_matched)
...
subset_with_clusters_AG_matched <- readRDS("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/rds/clinical_bifactor_results.rds")
...
clinical_measure_names <- names(subset_with_clusters_AG_matched)[grep("Bifactor", names(subset_with_clusters_AG_matched))] #get the names of all the columns with factorv2 in the name
...
{
#  lm(substitute(i ~ sex + age_in_years + ageSq + Hydra_k3, list(i = as.name(x))), data = subset_with_clusters_AG_matched)
   lm(substitute(i ~ Hydra_k3, list(i = as.name(x))), data = subset_with_clusters_AG_matched)
...
subset_with_clusters_AG_matched <- readRDS("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/rds/anxious_misery_results.rds")
clusters <- names(subset_with_clusters_AG_matched[grep ("Hydra", names(subset_with_clusters_AG_matched))])
...
clusters <- names(subset_with_clusters_AG_matched[grep ("Hydra", names(subset_with_clusters_AG_matched))])
parcellations <- names(subset_with_clusters_AG_matched[grep ("nback_func_sc", names(subset_with_clusters_AG_matched))])
...
write.csv(all_mean_sd_sem, "/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/all_mean_sd_sem_nback_unsorted.csv")

new_order <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/all_mean_sd_sem_nback_sorted.csv")
...
set.seed(1)
subset_with_clusters_AG_matched <- readRDS("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/rds/network_results.rds")

#Get cluster and parcellation names
clusters <- names(subset_with_clusters_AG_matched[grep ("Hydra", names(subset_with_clusters_AG_matched))])
parcellations <- names(subset_with_clusters_AG_matched[4:11])
subset_with_clusters_AG_matched$network_strength <- rowSums(subset_with_clusters_AG_matched[,4:11])
...
#  lm(substitute(i ~ sex + age_in_years + ageSq + restRelMeanRMSMotion + nbackRelMeanRMSMotion + idemoRelMeanRMSMotion + Hydra_k3, list(i = as.name(parcellation))), data = subset_with_clusters_AG_matched)
    lm(substitute(i ~ restRelMeanRMSMotion + nbackRelMeanRMSMotion + idemoRelMeanRMSMotion + Hydra_k3, list(i = as.name(parcellation))), data = subset_with_clusters_AG_matched)
})
names(parcellation_cluster_stats_lm_agesq_AG_matched) <- parcellations
...
network_strength_lm <- lm(network_strength~restRelMeanRMSMotion + nbackRelMeanRMSMotion + idemoRelMeanRMSMotion + Hydra_k3, data = subset_with_clusters_AG_matched)
...
num_clusters = 3
#Make sample
dem_and_cluster_data <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/df_task_active_node_strength_13roi_rest_20190321.csv")
...
#Get cluster and parcellation names
clusters <- names(subset_with_clusters_AG_matched[grep ("Hydra", names(subset_with_clusters_AG_matched))])
parcellations <- names(subset_with_clusters_AG_matched[2:14])
subset_with_clusters_AG_matched$network_strength <- rowSums(dem_and_cluster_data[,2:14])
#parcellations <- "network_strength"
...
#write.csv(all_mean_sd_sem, "/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/fc_task_active_13roi_unsorted.csv")
#plot
...
new_order <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/fc_task_active_13roi_sorted.csv")
...
#######Plotting average adj matrices by Subtype ###########
names_of_subtypes <- c("Typically Developing", "Subtype 1", "Subtype 2", "Subtype 3")
node_names <- c("Crus I Right", "Crus I Left", "Insula L", "DLPFC Anterior Left", "DLPFC Posterior Right", "Anterior Cingulate", "Medial Frontal Gyrus Left", "Medial Frontal Gyrus Right", "Thalamus Right", "Thalamus L", "Parietal Left", "Precuneus Left", "Precuneus Right")

averages <- readRDS("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/rds/averages_13roi_rest.rds")
td <- averages[[1]]
s1 <- averages[[2]]
s2 <- averages[[3]]
s3 <- averages[[4]]
...
mats <- list(td, s1, s2, s3, overall_mean)
lapply(mats, function(mat) {
  better_chorDiagram_13(adj = mat, node_names = node_names)
})
```
```{r concat_connectivity_13rois}
set.seed(1)
num_clusters = 3
#Make sample
dem_and_cluster_data <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/df_task_active_node_strength_13roi_concat_20190321.csv")
...
#Pull p-values
p_anova_lm_agesq <- sapply(parcellation_cluster_stats_anova_lm_agesq_AG_matched, function(v) v$"Pr(>F)"[4]) #$coef[,"Pr(>F)"][2]) #get the p value for dep binarized
...
filename <- Sys.glob(paste0("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/df_27rois_rest_alone_within_between_n333_20190329.csv"))
...
{
  lm(substitute(i ~ restRelMeanRMSMotion + nbackRelMeanRMSMotion + idemoRelMeanRMSMotion + Hydra_k3, list(i = as.name(network))), data = subset_with_clusters_AG_matched)
})
...
ggplot(subset_for_violin, aes(x = Hydra_k3, y = Task_active_Default_mode, fill = Hydra_k3)) +  
   geom_violin(trim = TRUE) + 
   xlab("Clusters") + ylab(paste0("R")) +
   ...
   theme(legend.position = "none",
    ...
    stat_summary(fun.data=mean_se, 
                 geom="pointrange") + 
    ggtitle(paste0("Network Strength, P=0.03"))  
  

ggplot(subset_for_violin, aes(x = Hydra_k3, y = Task_active_Default_mode, fill = Hydra_k3)) +  
   geom_jitter(aes(color=Hydra_k3), width = 0.1, size = 1) + 
   xlab("Clusters") + ylab(paste0("R")) +
   scale_y_continuous(limits=c(-0.5, 0.2)) + 
   theme(legend.position = "none",
          axis.text.x = element_text(size=15),
   ...
   geom_segment(aes(x = 0.85, y = all_mean_sd_sem_between$mean[2]), size = 1.2, xend = 1.15, yend = all_mean_sd_sem_between$mean[2]) +
   geom_segment(aes(x = 1.85, y = all_mean_sd_sem_between$mean[3]), size = 1.2, xend = 2.15, yend = all_mean_sd_sem_between$mean[3]) +
   geom_segment(aes(x = 2.85, y = all_mean_sd_sem_between$mean[4]), size = 1.2, xend = 3.15, yend = all_mean_sd_sem_between$mean[4]) +
   geom_segment(aes(x = 3.85, y = all_mean_sd_sem_between$mean[1]), size = 1.2, xend = 4.15, yend = all_mean_sd_sem_between$mean[1]) +
    #coord_fixed(ratio = 0.12) +
    ggtitle(paste0("Between network connectivity, P=0.03"))#, network_strength_overall_p)) 
...
ggplot(subset_for_violin, aes(x = Hydra_k3, y = Task_active_Default_mode, fill = Hydra_k3)) +  
   ...
   theme(legend.position = "none",
          ...
          axis.title.x = element_blank(),
          plot.title = element_text(size = 15)) +
   geom_segment(aes(x = 0.85, y = all_mean_sd_sem_between$mean[1]), size = 1.2, xend = 1.15, yend = all_mean_sd_sem_between$mean[1]) +
   geom_segment(aes(x = 1.85, y = all_mean_sd_sem_between$mean[2]), size = 1.2, xend = 2.15, yend = all_mean_sd_sem_between$mean[2]) +
   geom_segment(aes(x = 2.85, y = all_mean_sd_sem_between$mean[3]), size = 1.2, xend = 3.15, yend = all_mean_sd_sem_between$mean[3]) +
   geom_segment(aes(x = 3.85, y = all_mean_sd_sem_between$mean[4]), size = 1.2, xend = 4.15, yend = all_mean_sd_sem_between$mean[4]) +
  #  coord_fixed(ratio = 0.12) +
    ggtitle(paste0("Between network connectivity, P=0.03")) #, network_strength_overall_p)) 
...
data=subset_for_violin
data$MeanValue=c(1:length(data$Hydra_k3))
data$MeanValue[which(data$Hydra_k3=='TD')]=mean(data$Task_active_Default_mod[which(data$Hydra_k3=='TD')])
data$MeanValue[which(data$Hydra_k3=='Subtype 1')]=mean(data$Task_active_Default_mod[which(data$Hydra_k3=='Subtype 1')])
data$MeanValue[which(data$Hydra_k3=='Subtype 2')]=mean(data$Task_active_Default_mod[which(data$Hydra_k3=='Subtype 2')])
data$MeanValue[which(data$Hydra_k3=='Subtype 3')]=mean(data$Task_active_Default_mod[which(data$Hydra_k3=='Subtype 3')])

ggplot(data, aes(Hydra_k3, Task_active_Default_mode,col=Hydra_k3)) +
