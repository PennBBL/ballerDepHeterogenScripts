...
volume_score_cluster_stats_lm_agesq_AG_matched_by_cluster_1through10 <- lapply(cluster_names_list, function(cluster_name)
{
  ...
  {
    volume_measure <- as.name(volume_measure_name)
    lm(substitute(volume_measure ~ cluster + sex + ageSq + averageManualRating, list(volume_measure = volume_measure, cluster = cluster)), data = subset_with_clusters_AG_matched)
  })
...
volume_score_cluster_stats_anova_lm_agesq_AG_matched_by_cluster_1through10 <- lapply(cluster_names_list, function(cluster_name)
{
  ...
  {
     volume_measure <- as.name(volume_measure_name)
     anova(lm(substitute(volume_measure ~ cluster + sex + ageSq + averageManualRating, list(volume_measure = volume_measure, cluster = cluster)), data = subset_with_clusters_AG_matched))
  })
