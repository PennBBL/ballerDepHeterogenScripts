...
set currdir = `pwd`
set wkdir = "/data/jux/BBL/projects/ballerDepHeterogen/data/neuroimaging/processed_data/concat_restbold_nback_idemo/rest_nback_idemo_14FunctionalROIs_2mm/"
set oldts = "/data/jux/BBL/projects/ballerDepHeterogen/data/neuroimaging/processed_data/concat_restbold_nback_idemo/concat_rest_nback_idemo_2mm/ts_files_from_3dROIstats/"
set newts = $wkdir/ts_files_from_3dROIstats_14rois_2mm
...
foreach ts (`ls $oldts/*.txt`)
	...
	cp $ts ${newts}/${ts_root}_14roi_2mm.txt
	#change format of new file so it is single space delimited
	more ${newts}/${ts_root}_14roi_2mm.txt | sed -e "s/\s\{1,\}/ /g" >! temp
	mv temp ${newts}/${ts_root}_14roi_2mm.txt	
	Rscript call_27roi_to_14_roi_r_script.R ${newts}/${ts_root}_14roi_2mm.txt
end
