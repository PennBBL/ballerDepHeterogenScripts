...
set currdir = `pwd`
set wkdir = "/data/jux/BBL/projects/ballerDepHeterogen/data/neuroimaging/processed_data/concat_restbold_nback_idemo/rest_nback_idemo_14FunctionalROIs/"
set oldts = $wkdir/ts_files_from_3dROIstats
set newts = $wkdir/ts_files_from_3dROIstats_14rois

#copy old files into this directory
foreach ts (`ls $oldts/*.txt`)
	set ts_root = `echo $ts:r:t`
	cp $ts ${newts}/${ts_root}_14roi.txt
	#change format of new file so it is single space delimited
	more ${newts}/${ts_root}_14roi.txt | sed -e "s/\s\{1,\}/ /g" >! temp
	mv temp ${newts}/${ts_root}_14roi.txt	
	Rscript call_27roi_to_14_roi_r_script.R ${newts}/${ts_root}_14roi.txt
end
