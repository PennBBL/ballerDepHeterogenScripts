---
title: "Node strength, task deactivation, matched age and sex, n312, Hydra K3, 6 jneurosci labels, 2mm"
author: "Erica Baller"
date: "02/05/2019"
output:
...
dem_and_cluster_data <- read.csv("/Users/eballer/BBL/from_chead/ballerDepHeterogen/results/csvs/df_task_deactive_node_strength_6roi_20190206.csv")
...
ggplot(data = all_mean_sd_sem, aes(x = newNames, y = mean, group = cl)) + ylab("% Signal Change") +  
  ggtitle(paste0("Node strength Task DeActive Areas, overall strength P(FDR) = ", network_strength_overall_p)) + 
...
ggplot(data = all_mean_sd_sem, aes(x = newNames, y = mean, group = cl)) + ylab("Node Strength") +  
  ggtitle("Node Strength in Task Deactive Areas based on N-back Functional Regions of Interest") + 
  geom_line(aes(color=cl, size=.1), show.legend = F) +
#  geom_line(aes(color=cl), show.legend = F) +
  geom_point(aes(color=cl)) + 
  geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.1, size=1.5) +
