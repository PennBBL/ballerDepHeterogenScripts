...
#covaData<-read.csv("subset_with_T1_NbackFC_and_dem_with_clusters.csv", header = TRUE, sep = " ")
...
inputData <- read.csv(inputPath, sep = " ", header = TRUE)
dataSubj <- merge(covaData, inputData, by=subjID)

#############################################################################
#################Make hydra into factors#####################################
#############################################################################
hydra_names <- names(dataSubj[107:116])
clusters <- names(dataSubj[grep ("Hydra", names(dataSubj))])
dataSubj[hydra_names] <- lapply(dataSubj[clusters], factor)
dataSubj$sex <- as.factor(dataSubj$sex)
dataSubj$race <- as.factor(dataSubj$race)
...
OutDir <- paste0(OutDirRoot, "/n",dim(dataSubj)[1],"_rds_",subjDataOut,"_inclusion_",inclusionName, "_correction_", pAdjustMethod, "_ROI_",inputPathOut)
dir.create(OutDir)
setwd(OutDir)
