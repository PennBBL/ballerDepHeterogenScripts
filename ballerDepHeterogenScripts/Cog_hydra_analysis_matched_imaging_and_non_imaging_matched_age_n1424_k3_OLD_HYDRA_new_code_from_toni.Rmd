...
cnb_data_acc_sd_sem <- data.frame(cl = c("TD", "Cluster1", "Cluster2", "Cluster3"), mean_acc = c(mean(subset_with_clusters_AG_matched$NAR_Overall_Accuracy[which(subset_with_clusters_AG_matched$Hydra_k3 ==-1)]),
                                  ...
                                  sem = c(sd(subset_with_clusters_AG_matched$NAR_Overall_Accuracy[which(subset_with_clusters_AG_matched$Hydra_k3 == -1)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == -1))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Accuracy[which(subset_with_clusters_AG_matched$Hydra_k3 ==1)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 1))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Accuracy[which(subset_with_clusters_AG_matched$Hydra_k3 ==2)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 2))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Accuracy[which(subset_with_clusters_AG_matched$Hydra_k3 ==3)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 3)))))
...
cnb_data_speed_sd_sem <- data.frame(cl = c("TD", "Cluster1", "Cluster2", "Cluster3"), mean_speed = c(mean(subset_with_clusters_AG_matched$NAR_Overall_Speed[which(subset_with_clusters_AG_matched$Hydra_k3 ==-1)]),
                                    ...
                                    sem = c(sd(subset_with_clusters_AG_matched$NAR_Overall_Speed[which(subset_with_clusters_AG_matched$Hydra_k3 == -1)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == -1))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Speed[which(subset_with_clusters_AG_matched$Hydra_k3 ==1)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 1))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Speed[which(subset_with_clusters_AG_matched$Hydra_k3 ==2)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 2))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Speed[which(subset_with_clusters_AG_matched$Hydra_k3 ==3)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 3)))))
...
cnb_data_eff_sd_sem <- data.frame(cl = c("TD", "Cluster1", "Cluster2", "Cluster3"), mean_eff = c(mean(subset_with_clusters_AG_matched$NAR_Overall_Efficiency[which(subset_with_clusters_AG_matched$Hydra_k3 ==-1)]),
                                  ...
                                  sem = c(sd(subset_with_clusters_AG_matched$NAR_Overall_Efficiency[which(subset_with_clusters_AG_matched$Hydra_k3 == -1)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == -1))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Efficiency[which(subset_with_clusters_AG_matched$Hydra_k3 ==1)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 1))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Efficiency[which(subset_with_clusters_AG_matched$Hydra_k3 ==2)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 2))),
                                             sd(subset_with_clusters_AG_matched$NAR_Overall_Efficiency[which(subset_with_clusters_AG_matched$Hydra_k3 ==3)])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == 3)))))
...
cnb_all_measures <- data.frame(cl=c("TD", "Cluster 1", "Cluster 2","Cluster 3"), accuracy = df_mean_accuracy_z, speed = df_mean_processing_speed_z, efficiency = df_mean_efficiency_z)
...
ggplot(data = cnb_measures_for_plot, aes(x = cnb, y = z_score, group = cluster)) + ylab("Mean Z-score") + xlab("Summary Scores") + 
  ggtitle("CNB Summary Factor Scores") +
  geom_line(aes(color=cluster, size=.1), show.legend = F) +
  geom_point(aes(color=cluster)) + 
  geom_errorbar(aes(ymin=z_score-sem, ymax=z_score+sem), width=.1, size=1.5) +
  scale_x_discrete(labels= summary_scores_first_letter_cap, name = " ") +
...
for(cnb in cnb_measure_names_list){
  for(num in 1:4) {
    ...
    sem_for_eval <- paste("sd(subset_with_clusters_AG_matched$", cnb, "[which(subset_with_clusters_AG_matched$Hydra_k3 == ", groups_list[num,2], ")])/sqrt(length(which(subset_with_clusters_AG_matched$Hydra_k3 == ", groups_list[num,2], ")))", sep="")
...
#ggplot(data = all_mean_sd_sem, aes(x = cnb, y = mean, group = cluster)) +  ylab("Mean Z-score") +
 # geom_line(aes(color=cluster)) +
#  geom_point(aes(color=cluster)) + 
...
groups_list <- data.frame(cbind(c("TD", "Cluster1", "Cluster2", "Cluster3"), c("-1", "1", "2", "3")))
names(groups_list) <- c("cl", "numeric")
all_mean_sd_sem <- data.frame(rep(groups_list$cl, length(cnb_accuracy_names)), rep(cnb_accuracy_names, each = 4), rep(0, 1), rep(0, 1), rep(0,1))
names(all_mean_sd_sem) <- c("cl", "cnb", "mean", "sd", "sem")
...
#ggplot(data = all_mean_sd_sem, aes(x = cnb, y = mean, group = cluster)) +  ylab("Mean Z-score") + xlab("Accuracy") +
...
groups_list <- data.frame(cbind(c("TD", "Cluster1", "Cluster2", "Cluster3"), c("-1", "1", "2", "3")))
names(groups_list) <- c("cl", "numeric")
all_mean_sd_sem <- data.frame(rep(groups_list$cl, length(cnb_speed_names)), rep(cnb_speed_names, each = 4), rep(0, 1), rep(0, 1), rep(0,1))
names(all_mean_sd_sem) <- c("cl", "cnb", "mean", "sd", "sem")
...
ggplot(data = all_mean_sd_sem, aes(x = cnb, y = mean, group = cluster)) + ylab("Mean Z-score") + xlab("Summary Scores") + 
  ...
  geom_point(aes(color=cluster)) + 
  geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.1, size=1.5) +
  scale_x_discrete(labels=speed_names_no_s_z) +
  guides(colour = guide_legend(override.aes = list(size=10))) +
  theme(axis.text.x = element_text(size = 30, angle=60, hjust = 1), 
        axis.text.y = element_text(size = 30),
        title = element_text(size = 30), 
...
#ggplot(data = all_mean_sd_sem, aes(x = cnb, y = mean, group = cluster)) +  ylab("Mean Z-score") + xlab("Speed") + 
 # scale_x_discrete(labels=speed_names_no_s_z) +
  #geom_line(aes(color=cluster)) +
  #geom_point(aes(color=cluster)) + 
  #geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.1) +
